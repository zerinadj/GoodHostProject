...
using GoodHost.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace GoodHost.Tests
{
    public abstract class GoodHostTestBase : AbpIntegratedTestBase<GoodHostTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GoodHostTestBase()
        {
        ...
        protected void UsingDbContext(Action<GoodHostDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GoodHostDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GoodHostDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GoodHostDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GoodHostDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GoodHostDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GoodHostDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GoodHostDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GoodHostDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GoodHostDbContext, Task<T>> func)
        {
            T result;
